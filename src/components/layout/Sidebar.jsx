...
const Sidebar = () => {
    const { getUsers, users, isUsersLoading, getMessagesFromTo, isMessagesLoading } = useChatStore();
    ...
    return (
        <div className='bg-base-100 rounded-lg h-full'>
            ...
            {users.map((user, i)=> (
                <button key={i} onClick={()=> getMessagesFromTo(user.id)} type='button' className='flex items-center gap-2 px-4 py-3 cursor-pointer select-none hover:bg-base-200 w-full'>
